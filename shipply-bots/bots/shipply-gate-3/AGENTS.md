# Agent Instructions for shipply-gate-3

This file guides AI assistants working on the `shipply-gate-3` bot handler.

## Bot overview

`bots/shipply-gate-3/src/shipply_gate_3/shipply_gate_3.py` is a Pacto bot handler built on the
...
```bash
cd bots/shipply-gate-3
python -m shipply_gate_3
```

Tests import the public API via `from shipply_gate_3 import bot`.
...
After adding commands, update the tests in `bots/shipply-gate-3/tests/test_bot.py`
if necessary.
