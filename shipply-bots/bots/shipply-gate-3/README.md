...
- /gate_three — TODO: implement /gate_three.
...
The generated `bots/shipply-gate-3/systemd.service` file runs the bot as a
...
2. Copy the bot to `/opt/shipply-gate-3` and install its dependencies:
   ```bash
   sudo cp -r bots/shipply-gate-3 /opt/shipply-gate-3
   sudo pip install /opt/shipply-gate-3
   sudo chown -R pacto-bot:pacto-bot /opt/shipply-gate-3
   ```

3. Install the systemd unit:
   ```bash
   sudo cp /opt/shipply-gate-3/systemd.service /etc/systemd/system/shipply-gate-3.service
   sudo systemctl daemon-reload
   sudo systemctl enable --now shipply-gate-3
   ```

4. Check status and logs:
   ```bash
   sudo systemctl status shipply-gate-3
   sudo journalctl -u shipply-gate-3 -f
   ```
...
- When running in Docker Compose, follow the bot container logs:
  ```bash
  docker logs -f shipply-gate-3-shipply-gate-3-1
  ```
...
```bash
PACTO_LOG_LEVEL=debug python -m shipply_gate_3
```
...
```bash
sudo systemctl edit shipply-gate-3
```
...
```bash
sudo systemctl daemon-reload
sudo systemctl restart shipply-gate-3
```

## Environment variables

Copy `bots/shipply-gate-3/.env.example` to `bots/shipply-gate-3/.env` and fill in the
