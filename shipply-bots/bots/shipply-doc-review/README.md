...
The generated `bots/shipply-doc-review/systemd.service` file runs the bot as a
...
2. Copy the bot to `/opt/shipply-doc-review` and install its dependencies:
   ```bash
   sudo cp -r bots/shipply-doc-review /opt/shipply-doc-review
   sudo pip install /opt/shipply-doc-review
   sudo chown -R pacto-bot:pacto-bot /opt/shipply-doc-review
   ```

3. Install the systemd unit:
   ```bash
   sudo cp /opt/shipply-doc-review/systemd.service /etc/systemd/system/shipply-doc-review.service
   sudo systemctl daemon-reload
   sudo systemctl enable --now shipply-doc-review
   ```

4. Check status and logs:
   ```bash
   sudo systemctl status shipply-doc-review
   sudo journalctl -u shipply-doc-review -f
   ```
...
- When running in Docker Compose, follow the bot container logs:
  ```bash
  docker logs -f shipply-doc-review-shipply-doc-review-1
  ```
...
```bash
PACTO_LOG_LEVEL=debug python -m shipply_doc_review
```
...
```bash
sudo systemctl edit shipply-doc-review
```
...
```bash
sudo systemctl daemon-reload
sudo systemctl restart shipply-doc-review
```

## Environment variables

Copy `bots/shipply-doc-review/.env.example` to `bots/shipply-doc-review/.env` and fill in the
