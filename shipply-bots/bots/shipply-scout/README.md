...
The generated `bots/shipply-scout/systemd.service` file runs the bot as a
...
2. Copy the bot to `/opt/shipply-scout` and install its dependencies:
   ```bash
   sudo cp -r bots/shipply-scout /opt/shipply-scout
   sudo pip install /opt/shipply-scout
   sudo chown -R pacto-bot:pacto-bot /opt/shipply-scout
   ```

3. Install the systemd unit:
   ```bash
   sudo cp /opt/shipply-scout/systemd.service /etc/systemd/system/shipply-scout.service
   sudo systemctl daemon-reload
   sudo systemctl enable --now shipply-scout
   ```

4. Check status and logs:
   ```bash
   sudo systemctl status shipply-scout
   sudo journalctl -u shipply-scout -f
   ```
...
- When running in Docker Compose, follow the bot container logs:
  ```bash
  docker logs -f shipply-scout-shipply-scout-1
  ```
...
```bash
PACTO_LOG_LEVEL=debug python -m shipply_scout
```
...
```bash
sudo systemctl edit shipply-scout
```
...
```bash
sudo systemctl daemon-reload
sudo systemctl restart shipply-scout
```

## Environment variables

Copy `bots/shipply-scout/.env.example` to `bots/shipply-scout/.env` and fill in the
