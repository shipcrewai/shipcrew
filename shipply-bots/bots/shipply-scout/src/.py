"""Bot handler for shipply-scout."""

from __future__ import annotations

from pacto_bot_sdk import Bot, parse_command

bot = Bot(bot_id="shipply-scout")
...
async def start_handler(event, bot):
    bot.log(f"received /start: event_id={event.event_id}")
    response = {
        "event_id": event.event_id,
        "action": "reply",
        "content": "start placeholder response",
    }
    bot.log(f"handled /start: action={response['action']}")
    return response
