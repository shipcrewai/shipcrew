"""Bot handler for shipply-gate-2."""

from __future__ import annotations

from pacto_bot_sdk import Bot, parse_command

bot = Bot(bot_id="shipply-gate-2")
...
@bot.command("/authorize")
async def authorize_handler(event, bot):
    bot.log(f"received /authorize: event_id={event.event_id}")
    response = {
        "event_id": event.event_id,
        "action": "reply",
        "content": "authorize placeholder response",
    }
    bot.log(f"handled /authorize: action={response['action']}")
    return response
