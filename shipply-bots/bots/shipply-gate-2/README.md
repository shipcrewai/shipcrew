...
- /authorize — TODO: implement /authorize.
...
The generated `bots/shipply-gate-2/systemd.service` file runs the bot as a
...
2. Copy the bot to `/opt/shipply-gate-2` and install its dependencies:
   ```bash
   sudo cp -r bots/shipply-gate-2 /opt/shipply-gate-2
   sudo pip install /opt/shipply-gate-2
   sudo chown -R pacto-bot:pacto-bot /opt/shipply-gate-2
   ```

3. Install the systemd unit:
   ```bash
   sudo cp /opt/shipply-gate-2/systemd.service /etc/systemd/system/shipply-gate-2.service
   sudo systemctl daemon-reload
   sudo systemctl enable --now shipply-gate-2
   ```

4. Check status and logs:
   ```bash
   sudo systemctl status shipply-gate-2
   sudo journalctl -u shipply-gate-2 -f
   ```
...
- When running in Docker Compose, follow the bot container logs:
  ```bash
  docker logs -f shipply-gate-2-shipply-gate-2-1
  ```
...
```bash
PACTO_LOG_LEVEL=debug python -m shipply_gate_2
```
...
```bash
sudo systemctl edit shipply-gate-2
```
...
```bash
sudo systemctl daemon-reload
sudo systemctl restart shipply-gate-2
```

## Environment variables

Copy `bots/shipply-gate-2/.env.example` to `bots/shipply-gate-2/.env` and fill in the
