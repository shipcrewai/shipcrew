# Agent Instructions for shipply-forge

This file guides AI assistants working on the `shipply-forge` bot handler.

## Bot overview

`bots/shipply-forge/src/shipply_forge/shipply_forge.py` is a Pacto bot handler built on the
...
```bash
cd bots/shipply-forge
python -m shipply_forge
```

Tests import the public API via `from shipply_forge import bot`.
...
After adding commands, update the tests in `bots/shipply-forge/tests/test_bot.py`
if necessary.
