...
- /execute — TODO: implement /execute.
...
The generated `bots/shipply-forge/systemd.service` file runs the bot as a
...
2. Copy the bot to `/opt/shipply-forge` and install its dependencies:
   ```bash
   sudo cp -r bots/shipply-forge /opt/shipply-forge
   sudo pip install /opt/shipply-forge
   sudo chown -R pacto-bot:pacto-bot /opt/shipply-forge
   ```

3. Install the systemd unit:
   ```bash
   sudo cp /opt/shipply-forge/systemd.service /etc/systemd/system/shipply-forge.service
   sudo systemctl daemon-reload
   sudo systemctl enable --now shipply-forge
   ```

4. Check status and logs:
   ```bash
   sudo systemctl status shipply-forge
   sudo journalctl -u shipply-forge -f
   ```
...
- When running in Docker Compose, follow the bot container logs:
  ```bash
  docker logs -f shipply-forge-shipply-forge-1
  ```
...
```bash
PACTO_LOG_LEVEL=debug python -m shipply_forge
```
...
```bash
sudo systemctl edit shipply-forge
```
...
```bash
sudo systemctl daemon-reload
sudo systemctl restart shipply-forge
```

## Environment variables

Copy `bots/shipply-forge/.env.example` to `bots/shipply-forge/.env` and fill in the
