...
The generated `bots/shipply-gate-1/systemd.service` file runs the bot as a
...
2. Copy the bot to `/opt/shipply-gate-1` and install its dependencies:
   ```bash
   sudo cp -r bots/shipply-gate-1 /opt/shipply-gate-1
   sudo pip install /opt/shipply-gate-1
   sudo chown -R pacto-bot:pacto-bot /opt/shipply-gate-1
   ```

3. Install the systemd unit:
   ```bash
   sudo cp /opt/shipply-gate-1/systemd.service /etc/systemd/system/shipply-gate-1.service
   sudo systemctl daemon-reload
   sudo systemctl enable --now shipply-gate-1
   ```

4. Check status and logs:
   ```bash
   sudo systemctl status shipply-gate-1
   sudo journalctl -u shipply-gate-1 -f
   ```
...
- When running in Docker Compose, follow the bot container logs:
  ```bash
  docker logs -f shipply-gate-1-shipply-gate-1-1
  ```
...
```bash
PACTO_LOG_LEVEL=debug python -m shipply_gate_1
```
...
```bash
sudo systemctl edit shipply-gate-1
```
...
```bash
sudo systemctl daemon-reload
sudo systemctl restart shipply-gate-1
```

## Environment variables

Copy `bots/shipply-gate-1/.env.example` to `bots/shipply-gate-1/.env` and fill in the
