# Agent Instructions for shipply-gate-1

This file guides AI assistants working on the `shipply-gate-1` bot handler.

## Bot overview

`bots/shipply-gate-1/src/shipply_gate_1/shipply_gate_1.py` is a Pacto bot handler built on the
...
```bash
cd bots/shipply-gate-1
python -m shipply_gate_1
```

Tests import the public API via `from shipply_gate_1 import bot`.
...
After adding commands, update the tests in `bots/shipply-gate-1/tests/test_bot.py`
if necessary.
